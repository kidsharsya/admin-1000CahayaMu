...
import { createElectricityCategory } from '@/lib/api/electricityCategory';

interface GolonganListrikFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function GolonganListrikFormModal({ isOpen, onClose, onSuccess }: GolonganListrikFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createElectricityCategory(form);
      setShowSuccess(true);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Tambah Golongan Listrik</h2>

        {showSuccess ? (
          <div className="text-center py-6">
            <h3 className="text-lg font-semibold text-emerald-600 mb-2">Berhasil!</h3>
            <p className="text-gray-700 mb-4">Golongan listrik berhasil ditambahkan.</p>
            <button
              onClick={() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="category_name" value={form.category_name} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>
