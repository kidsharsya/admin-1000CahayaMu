...
import { deleteFAQCategory } from '@/lib/api/faq';
import { useState } from 'react';

interface FAQCategoryDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  category: {
...
export function FAQCategoryDeleteModal({ isOpen, onClose, onSuccess, category, itemCount }: FAQCategoryDeleteModalProps) {
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        {!showSuccess && (
          <>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-gray-800">Hapus Kategori FAQ</h2>
            </div>

            <p className="text-gray-700 mb-4">
              Apakah kamu yakin ingin menghapus kategori <span className="font-semibold text-gray-900">&quot;{category.Name}&quot;</span>?
            </p>

            {/* Warning jika ada items */}
            {itemCount > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4 flex items-start gap-2">
                <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                <div className="text-sm">
                  <p className="font-medium text-amber-800 mb-1">Perhatian!</p>
                  <p className="text-amber-700">
                    Terdapat <strong>{itemCount} FAQ item</strong> dalam kategori ini. Semua item akan ikut terhapus.
                  </p>
                </div>
              </div>
            )}
        ...
        {showSuccess && (
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">Kategori FAQ berhasil dihapus dari sistem.</p>
            <button
              onClick={() => {
