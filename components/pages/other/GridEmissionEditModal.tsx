...
export function GridEmissionEditModal({ isOpen, id, onClose, onSuccess }: GridEmissionEditModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await getGridEmissionFactorById(id);
        setForm({
          region_name: data.region_name,
          co2e_per_kwh: data.co2e_per_kwh,
          active: data.active,
        });
      } catch (err) {
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        {showSuccess ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Category Name */}
            <div>
              <label className="block text-sm text-gray-600 mb-1">Region Name</label>
              <input type="text" name="category_name" value={form.region_name} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>

            <div>
              <label className="block text-sm text-gray-600 mb-1">CO2e per kWh</label>
              <input type="number" name="co2e_per_kwh" value={form.co2e_per_kwh} onChange={handleChange} required min={0} step={1} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>
