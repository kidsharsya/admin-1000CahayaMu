import { Flame, BarChart3, FileText, Building2, Car, Wallet } from 'lucide-react';
...
export function EmissionStatsGrid({ total_emission, avg_emission, report_count, building_count, vehicle_count, total_electricity_spending_rp, total_fuel_spending_rp, loading = false }: EmissionStatsGridProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 gap-4">
        <EmissionStatsCard icon={<Wallet className="w-6 h-6 text-yellow-500" />} label="Total Biaya Listrik" value={loading ? '...' : total_electricity_spending_rp.toLocaleString('id-ID')} unit="Rupiah" color="bg-yellow-50" />

        <EmissionStatsCard icon={<Wallet className="w-6 h-6 text-pink-500" />} label="Total Biaya Bahan Bakar" value={loading ? '...' : total_fuel_spending_rp.toLocaleString('id-ID')} unit="Rupiah" color="bg-pink-50" />
      </div>
    </div>
  );
}
