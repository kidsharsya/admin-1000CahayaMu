...
import { createFuelPrice } from '@/lib/api/fuelPrice';
import { getFuelProducts } from '@/lib/api/fuelProduct';
import { FuelProductTypes } from '@/types/fuelProductType';

interface FuelPriceFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function FuelPriceFormModal({ isOpen, onClose, onSuccess }: FuelPriceFormModalProps) {
  ...
  // ✅ Hook harus dideklarasikan sebelum conditional return
  useEffect(() => {
  ...
  // Setelah semua hook, baru boleh ada conditional render
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Tambah Harga Bahan Bakar</h2>

        {showSuccess ? (
          <div className="text-center py-6">
            <h3 className="text-lg font-semibold text-emerald-600 mb-2">Berhasil!</h3>
            <p className="text-gray-700 mb-4">Data harga bahan bakar berhasil ditambahkan.</p>
            <button
              onClick={() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select name="fuel_product_id" value={form.fuel_product_id} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">
            ...
            <div>
              ...
              <input type="date" name="effective_date" value={form.effective_date} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>
