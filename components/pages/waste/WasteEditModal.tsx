...
import { getWasteById, updateWaste } from '@/lib/api/waste';
...
export function WasteEditModal({ isOpen, onClose, onSuccess, id, data }: WasteEditModalProps) {
    ...
    if (id) {
      (async () => {
        try {
          setFetching(true);
          setError(null);
          const fetched = await getWasteById(id);
          setForm({
            name: fetched.name ?? '',
            unit: fetched.unit ?? 'kg',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateWaste(wasteId, payload);
      setShowSuccess(true);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        ...
        <h2 className="text-lg font-semibold text-gray-800 mb-4">Edit Jenis Sampah</h2>

        {showSuccess ? (
          <div className="text-center">
            <h3 className="text-lg font-semibold text-emerald-600 mb-2">Berhasil!</h3>
            <p className="text-gray-700 mb-4">Data jenis sampah berhasil diperbarui.</p>
            <button
              onClick={() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" name="name" value={form.name} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>

            <div>
              <label className="block text-sm text-gray-600 mb-1">Unit</label>
              <select name="unit" value={form.unit} onChange={handleChange} className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">
            ...
            <div>
              ...
              <input type="number" step="0.001" name="co2e_per_unit" value={form.co2e_per_unit} onChange={handleChange} required className="w-full border rounded-md px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500" />
            </div>
